...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      // SM 算法
      AMCLLaser *mylaser;
      pf_vector_t mypose,hit,pose_fin;
      double obs_range, obs_bearing;
      double z;

      double alaph = 0.001;
      double e1(1e-15),e2(1e-15);
      double mu(0.01),p0(0.2),p1(0.5),p2(0.7),m(0.005);
      double Aredk,Predk,rk;
     //ds/dx
     Eigen::MatrixXd dSd(2, 3);
     dSd(0, 0) = 1;
     dSd(0, 1) = 0;
     dSd(1, 0) = 0;
     dSd(1, 1) = 1;
     //dM/dx
     Eigen::MatrixXd dMd(1, 2);

     // J矩阵
     Eigen::MatrixXd J_matrxi(360, 3);
     Eigen::MatrixXd J_re(1, 3);        //J过渡矩阵

      //I
      Eigen::MatrixXd I_matrix(3, 3);   //单位阵
      I_matrix<<1,0,0,0,1,0,0,0,1;

     // F(x)矩阵
     Eigen::MatrixXd F_x(360, 1);
     Eigen::MatrixXd F_x_new(360, 1);
     Eigen::MatrixXd F_x_new_2(360, 1);
     //H 矩阵 = J^T *J
     Eigen::MatrixXd H_matrix(3, 3);
     //B矩阵 = -J^T * F
     Eigen::MatrixXd B_matrix(3, 1);

     //位置偏移值
     Eigen::MatrixXd dk1(3, 1);
     Eigen::MatrixXd dk2(3, 1);
     Eigen::MatrixXd yk(3, 1);
     Eigen::MatrixXd sk(3, 1);
     Eigen::MatrixXd ds(3, 1);

     pose_fin.v[0] = p.pose.pose.position.x;
     pose_fin.v[1] = p.pose.pose.position.y;
     pose_fin.v[2] = hyps[max_weight_hyp].pf_pose_mean.v[2];

     ds<<pose_fin.v[0],pose_fin.v[1],pose_fin.v[2];

     mylaser = lasers_[laser_index];
     mypose = pf_vector_coord_add(mylaser->laser_pose,pose_fin);
     ...
     for(int kk = 0;kk<120;kk++)
     {
      for (int i = 0; i < 360; i += 1)
      {
        ...
        if(obs_range >= laser_scan->range_max || obs_range != obs_range)
        {
          J_matrxi(i,0)=0;J_matrxi(i,1)=0;J_matrxi(i,2)=0;
          F_x(i,0) = 0;
          continue;
        }

        // Compute the endpoint of the beam  //继续上面《概率机器人》P128的后半段公式，得到激光雷达点最远端的世界坐标
        hit.v[0] = mypose.v[0] + obs_range * cos(mypose.v[2] + obs_bearing);
        hit.v[1] = mypose.v[1] + obs_range * sin(mypose.v[2] + obs_bearing);
      

        // Convert to map grid coords.  //转换到栅格坐标
        int mi, mj;
        mi = MAP_GXWX(mylaser->map, hit.v[0]);
        mj = MAP_GYWY(mylaser->map, hit.v[1]);

        //超出最大范围，舍弃
        if(!MAP_VALID(mylaser->map, mi, mj)) 
        {
          F_x(i,0) = 0;
          J_matrxi(i,0)=0;J_matrxi(i,1)=0;J_matrxi(i,2)=0;
          continue;
            ...
            z = mylaser->map->cells[MAP_INDEX(mylaser->map,mi,mj)].occ_state;
            F_x(i,0) = 1-z;

            //计算矩阵
            //dS/dx
            dSd(0,2) = -1*obs_range * sin(mypose.v[2] + obs_bearing);
            dSd(1,2) = obs_range * cos(mypose.v[2] + obs_bearing);
            ...
            //计算周围16个点的概率
            double P[16];
            P[0] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi-1,mj-1)].occ_state;
            P[1] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi-1,mj)].occ_state;
            P[2] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi-1,mj+1)].occ_state;
            P[3] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi-1,mj+2)].occ_state;

            P[4] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi,mj-1)].occ_state;
            P[5] = z;
            P[6] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi,mj+1)].occ_state;
            P[7] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi,mj+2)].occ_state;

            P[8] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi+1,mj-1)].occ_state;
            P[9] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi+1,mj)].occ_state;
            P[10] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi+1,mj+1)].occ_state;
            P[11] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi+1,mj+2)].occ_state;

            P[12] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi+2,mj-1)].occ_state;
            P[13] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi+2,mj)].occ_state;
            P[14] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi+2,mj+1)].occ_state;
            P[15] = mylaser->map->cells[MAP_INDEX(mylaser->map,mi+2,mj+2)].occ_state;

            double abcd[4],Dabcd[4];
            ...
            {
              int llk = lk*4;
              abcd[lk] = (-0.5*P[llk]+1.5*P[llk+1]-1.5*P[llk+2]+0.5*P[llk+3])*y3+(P[llk]-2.5*P[llk+1]+2*P[llk+2]-0.5*P[llk+3])*y2+(-0.5*P[llk]+0.5*P[llk+2])*y1+P[llk+1];
              Dabcd[lk] = (-0.5*P[llk]+1.5*P[llk+1]-1.5*P[llk+2]+0.5*P[llk+3])*3*y2+(P[llk]-2.5*P[llk+1]+2*P[llk+2]-0.5*P[llk+3])*2*y1+(-0.5*P[llk]+0.5*P[llk+2]);
            }
            dMd(0, 0) = abcd[0]*(-1.5*x2+2*x1-0.5)+abcd[1]*(4.5*x2-5*x1)+abcd[2]*(-4.5*x2+4*x1)+abcd[3]*(1.5*x2-x1);
            dMd(0, 1) = Dabcd[0]*(-0.5*x3+x2-0.5*x1)+Dabcd[1]*(1.5*x3-2.5*x2+1)+Dabcd[2]*(-1.5*x3+2*x2+0.5*x1)+Dabcd[3]*(0.5*x3-0.5*x2);
            
            J_re = dMd*dSd;
            J_matrxi(i,0) = J_re(0,0);
            J_matrxi(i,1) = J_re(0,1);
            J_matrxi(i,2) = J_re(0,2);
            // ROS_INFO("%f,%f,%f,%f,_four_point_map",J_re(0,0),J_re(0,1),J_re(0,2),F_x(i,0));
        }


      H_matrix = J_matrxi.transpose()*J_matrxi;
      B_matrix = J_matrxi.transpose()*F_x;

      if(B_matrix.maxCoeff()<e1) break;  //梯度下降过慢，结束
      alaph = (mu*F_x.norm())/(1+F_x.norm());

      dk1 = (H_matrix+alaph*I_matrix).colPivHouseholderQr().solve(B_matrix);
      yk = ds + dk1;
      double angle,dis,xx,yy;
      int mmi,mmj;
      for(int i =0;i<360;i++)
      {
        dis= laser_scan->ranges[i];
        angle = laser_scan->angle_min + i*laser_scan->angle_increment;

        if(dis >= laser_scan->range_max || dis != dis)
        {
          F_x_new(i,0) = 0;
          continue;
        }
        xx = yk(0,0) + dis * cos(yk(2,0) + angle);
        yy = yk(1,0) + dis * sin(yk(2,0) + angle);

        mmi = MAP_GXWX(mylaser->map, hit.v[0]);
        mmj = MAP_GYWY(mylaser->map, hit.v[1]);
        //ROS_INFO("%d,%d,_pose_map",mi,mj);

        //超出最大范围，舍弃
        if(!MAP_VALID(mylaser->map, mmi, mmj)) 
        {
          F_x_new(i,0) = 0;
          continue;
        }
        ...
        {
            F_x_new(i,0) =1-mylaser->map->cells[MAP_INDEX(mylaser->map,mmi,mmj)].occ_state;
        }
      }

      dk2 = (H_matrix+alaph*I_matrix).colPivHouseholderQr().solve(J_matrxi.transpose()*F_x_new);
      // ds = (H_matrix+mu*I_matrix).colPivHouseholderQr().solve(B_matrix);
      // if(ds(0,0)!=ds(0,0) || ds(1,0)!=ds(1,0) || ds(2,0)!=ds(2,0)) break; //ds为nan
      // if(ds.norm()<=e2*ds_new.norm()) break; //ds变化量过小
      // ds_new = ds_new + ds;
      sk = dk1 + dk2;
      ...
      for(int i =0;i<360;i++)
      {
        ...
        xx = (yk(0,0)+dk2(0,0)) + dis * cos((yk(2,0)+dk2(2,0)) + angle);
        yy = (yk(1,0)+dk2(1,0)) + dis * sin((yk(2,0)+dk2(2,0)) + angle);
        ...
        {
            F_x_new_2(i,0) =1-mylaser->map->cells[MAP_INDEX(mylaser->map,mmi,mmj)].occ_state;
        }
      }

      Aredk = F_x.norm()-F_x_new_2.norm();
      Predk = F_x.norm()-(F_x+J_matrxi*dk1).norm()+F_x_new.norm()-(F_x_new+J_matrxi*dk2).norm();
      rk = Aredk/Predk;
      if(rk>=p0) ds = ds + sk;
      if(rk<p1) mu = 4*mu;
      else if(rk>=p1 && rk<=p2);
      ...
      {
        Eigen::MatrixXd adf(1,2);
        adf<<mu/4,m;
        mu = adf.maxCoeff();
      }
      // if(fabs(ds(0,0))<0.001 && fabs(ds(1,0)<0.001) && fabs(ds(2,0)<0.001)) break;
      // if(ds(0,0)!=ds(0,0) || ds(1,0)!=ds(1,0) || ds(2,0)!=ds(2,0)) break;
     }
      pose_fin.v[0] = ds(0,0);
      pose_fin.v[1] = ds(1,0);
      pose_fin.v[2] = ds(2,0);

      // ROS_INFO("%f,%f,%f",ds(0,0),ds(1,0),ds(2,0));
      
      p.pose.pose.position.x =pose_fin.v[0];
      p.pose.pose.position.y =pose_fin.v[1];
      q.setRPY(0, 0, pose_fin.v[2]);
      tf2::convert(q, p.pose.pose.orientation);
      
      hyps[max_weight_hyp].pf_pose_mean.v[0] = pose_fin.v[0];
      hyps[max_weight_hyp].pf_pose_mean.v[1] = pose_fin.v[1];
      hyps[max_weight_hyp].pf_pose_mean.v[2] = pose_fin.v[2];
      // ROS_INFO("%f,%f,%f_plusplus",hyps[max_weight_hyp].pf_pose_mean.v[0],hyps[max_weight_hyp].pf_pose_mean.v[1],hyps[max_weight_hyp].pf_pose_mean.v[2]);
