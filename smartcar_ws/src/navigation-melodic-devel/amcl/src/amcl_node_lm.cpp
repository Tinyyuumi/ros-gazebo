...
class AmclNode
{
  ...
  private:
    ...
    ros::Publisher pose_pub_origin_;
        ...
        first_reconfigure_call_(true)
{
  ...
  pose_pub_origin_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("amcl_pose_origin", 2, true);
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
     ...
     for(int kk = 0;kk<120;kk++)
     {
      ...
      // ds = H_matrix.colPivHouseholderQr().solve(B_matrix);
      if(kk==0){
        Eigen::MatrixXd adf(1,3);
        adf<<H_matrix(0,0),H_matrix(1,1),H_matrix(2,2);
        mu = alaph*adf.maxCoeff();
      }
      ...
      for(int i =0;i<360;i++)
      {
        ...
        xx = ds_new(0,0) + dis * cos(ds_new(2,0) + angle);
        yy = ds_new(1,0) + dis * sin(ds_new(2,0) + angle);
      ...
      double rho = F_x.norm()-F_x_new.norm()/((ds.transpose()*(mu*ds+B_matrix))(0,0));
      if(rho>0)
      {
        pose_fin.v[0] = ds_new(0,0);
        pose_fin.v[1] = ds_new(1,0);
        pose_fin.v[2] = ds_new(2,0);
        Eigen::MatrixXd adf(1,2);
        adf<<1/3,1-(2*rho-1)*(2*rho-1)*(2*rho-1);
        mu = mu*adf.maxCoeff();
        v = 2;
        // F_x = F_x_new;
      }
      ...
      ROS_INFO("%f,%f,%f_dsdsds",ds(0,0),ds(1,0),ds(2,0));
      ROS_INFO("%f,%f,%f_plusplus",ds_new(0,0),ds_new(1,0),ds_new(2,0));
      mypose = pf_vector_coord_add(mylaser->laser_pose,pose_fin);
      }
